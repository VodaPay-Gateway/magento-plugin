...
namespace VodaPay\VodaPay\Controller\VodaPay;
...
class Payment implements HttpGetActionInterface
{
    ...
    public const VODAPAY_PENDING    = 'PENDING';
    public const VODAPAY_AWAIT3DS   = 'AWAIT3DS';
    public const VODAPAY_CANCELLED  = 'CANCELLED';
    ...
    public const VODAPAY_VOIDED     = 'VOIDED';
        ...
        if ($incrementId) {
            ...
            if ($order->getStatus() !== $this->config->getCustomSuccessOrderStatus($storeId)) {
                if ($order->getId()) {
                    ...
                    $orderItem->setState($this->vodapayState);
                    $orderItem->setStatus($this->vodapayState);
                    $orderItem->save();
                }
            }
        }
    ...
    public function getCapturePayment(
        ...
        } elseif($this->vodapayState === self::VODAPAY_STARTED) {
    ...
    {
        if ($this->vodapayState == self::VODAPAY_AUTHORISED) {
            $payment = $order->getPayment();
    ...
    {
        if ($this->vodapayState === self::VODAPAY_CAPTURED || $this->vodapayState === self::VODAPAY_PURCHASED) {
            $payment = $order->getPayment();
            $payment->setLastTransId($paymentId);
            $payment->setTransactionId($paymentId);
            $payment->setAdditionalInformation(['paymentResult' => json_encode($paymentResult)]);
    ...
    public function resultValidator($result)
    {
        if (isset($result['errors']) && is_array($result['errors'])) {
            ...
            $this->error        = false;
            $this->vodapayState = $result[self::VODAPAY_EMBEDED]['payment'][0]['state'] ?? '';
    ...
    {
        $orderItems = $this->fetchOrder('state', self::VODAPAY_STARTED)->addFieldToFilter(
            'payment_id',
            null
        ...
        if ($orderItems) {
            ...
            foreach ($orderItems as $orderItem) {
                ...
                try {
                    if ($order->getPayment()->getMethod() !== 'vodapay') {
                    ...
                    if ($result && isset($result[$embedded]['payment']) && is_array($result[$embedded]['payment'])) {
                        ...
                        if ($paymentResult['state'] === self::VODAPAY_STARTED
                            || $paymentResult['state'] === self::VODAPAY_AWAIT3DS
                            || $paymentResult['state'] == self::VODAPAY_PENDING
                            || $paymentResult['state'] == self::VODAPAY_CANCELLED
                        ) {
