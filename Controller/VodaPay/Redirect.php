...
use VodaPay\VodaPay\Block\VodaPay;

/**
 * Class Redirect
 *
 * The Redirect Controller responsible for sending the customer to the VodaPay Payment Page
 */
class Redirect implements HttpGetActionInterface
{
    ...
    /**
     * Redirects to vodapay payment portal
     *
    ...
    public function execute()
    {
        ...
        $vodapayPaymentAction = $this->config->getPaymentAction($storeId);

        $url = [];
        try {
            $block = $this->layoutFactory->create()->createBlock(VodaPay::class);
            $url   = $block->getPaymentUrl($vodapayPaymentAction);
        ...
        if (isset($url['url'])) {
            ...
            $order->setStatus('vodapay_failed');
