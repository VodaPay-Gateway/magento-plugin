...
class PaymentTransaction implements ClientInterface
{
    ...
    public function placeRequest(array|TransferInterface $requestData): ?array
    {
        ...
        $vodapayHttpTransfer = new NgeniusHTTPTransfer($url, $this->config->getHttpVersion($storeId));
        $vodapayHttpTransfer->setPaymentHeaders($token);
        $vodapayHttpTransfer->setMethod($method);
        $vodapayHttpTransfer->setData($data);

        $response = NgeniusHTTPCommon::placeRequest($vodapayHttpTransfer);
